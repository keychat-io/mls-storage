...
impl SqliteStorage {
    pub fn insert(&self, key: &Vec<u8>, value: &Vec<u8>) -> Result<(), String> {
        let sql = format!(
            "INSERT OR REPLACE INTO {} (iden_key, iden_value) values(?, ?)",
        ...
        //     sqlx::query(&sql).bind(key).bind(value).execute(&self.pool.db).await;
        // });
        // let sql2 = format!("INSERT OR REPLACE INTO {} (iden_key, iden_value) values({:?}, {:?})", self.pool.definition_identity(), key, value);
        // stdout().write_all(format!("{0}\n", sql2).as_bytes()).unwrap();
        {
        ...
        // stdout().write_all(b"a aaa  \n").unwrap();
        // let res = tokio::spawn( async move {
        //     stdout().write_all(b"tokio\n").unwrap();
        //     let sql  = sqlx::query(&sql).bind(key).bind(value);
        //
    ...
    pub fn del_value(&self, key: &Vec<u8>) -> Result<(), String> {
        ...
        // let key = key.clone();
        // let res = tokio::spawn( async move {
        //     let sql  = sqlx::query(&sql).bind(key);
        //
        //     let r =  sql.execute(&db).await;
        //     mp.send(r).expect("TODO: panic message");
        // });
    ...
    pub fn get_value(&self, key: &Vec<u8>) -> Result<Option<Vec<u8>>, String> {
        let sql = format!(
            "select iden_value from {} where iden_key = ?",
            self.pool.definition_identity()
        );
